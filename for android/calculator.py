...
value_list = {'颤颤之齿':3.49,'苦盐簇':2.31,'破碎骨片':2.04,'银原矿石':1.86,'清扫咒':1.57 ,'利齿子儿':0.00286,'启寤I':1/400,
              ...
              '百灵百验鸟':25.09,'祝圣秘银':32.80,'双头形骨架':31.95,'盐封曼德拉':35.88,'啮咬盒':37.68,'金爪灵摆':47.39,
...
dirc_list = {'颤颤之齿':1,'苦盐簇':2,'破碎骨片':3,'银原矿石':4,'清扫咒':5,'利齿子儿':37,'启寤I':38,
              ...
              '百灵百验鸟':14,'祝圣秘银':15,'双头形骨架':16,'盐封曼德拉':17,'啮咬盒':18,'金爪灵摆':19,
...
def clock(times):
    ...
    strs = strs + str(int(times)) + '秒'
    return strs


def solve(needs,sys_function:SysFuntion):
    global getsolve
    needs_sum = {}
    for i in needs:
    ...
    for perm2 in perm:
        ...
        sys_function.args = '完成{0:.2f}%\n预计还需'.format(sums / len(perm) * 100)+clock(t)
        sums += 1
        for i in perm2.copy():
            if Materials2[i].next:
                getsolve = True
